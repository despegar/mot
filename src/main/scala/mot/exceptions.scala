...
class ErrorStateException(cause: Throwable) 
  extends Exception(
      "Cannot send message because the connector is in error state and the client is configured as pessimistic", 
      cause)
...
class TooLateException(val delay: Long) 
  extends Exception("Cannot send respond because request timed out. Delay: " + delay)
